...
public sealed class AppState
{
    ...
    public static string DefaultFilePath() => Path.Combine(
        ...
        "PwAssistant", "accounts.json");

    /// <summary>Pre-rename location, imported once when the new one is empty.</summary>
    ...
    public async Task LoadAsync()
    {
        ...
        {
            Data = await _store.LoadAsync(LegacyFilePath()).ConfigureAwait(false);
            await _store.SaveAsync(_filePath, Data).ConfigureAwait(false);
        }
